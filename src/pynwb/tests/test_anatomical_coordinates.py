...
def test_atlas_registration_result_links_write_read(tmp_path):
    """AtlasRegistration links to its two result objects:

    - coordinates_image: an AnatomicalCoordinatesImage holding per-pixel CCF coordinates
      in the *registered* (atlas) space, i.e. the dense coordinate map after applying the
      affine transformation.
    - brain_region_masks: a BrainRegionMasks table mapping pixels to atlas region IDs in
      the *source* imaging space, i.e. atlas region boundaries projected back onto the
      original FOV without warping the image.
    """
    nwbfile = mock_NWBFile()
    localization = Localization()
    nwbfile.add_lab_meta_data([localization])
    ...
    atlas_space = AllenCCFv3Space()
    localization.add_spaces([atlas_space])

    # Dense CCF coordinate map for the registered FOV (atlas space)
    registered_coords = AnatomicalCoordinatesImage(
        name="RegisteredCoords",
        imaging_plane=mock_ImagingPlane(nwbfile=nwbfile),
        method="affine registration to Allen CCFv3",
        space=atlas_space,
        x=np.ones((5, 5)) * 5700.0,   # AP coordinate in um
        y=np.ones((5, 5)) * 4000.0,   # DV coordinate in um
        z=np.ones((5, 5)) * 5600.0,   # ML coordinate in um
    )
    localization.add_anatomical_coordinates_images([registered_coords])
    ...
    registration = AtlasRegistration(
        source_image=image_collection["SourceImage"],
        anatomical_coordinates_images=[registered_coords],
        brain_region_masks=[source_region_masks],
    )
    nwbfile.add_lab_meta_data([registration])

    with NWBHDF5IO(tmp_path / "test_result_links.nwb", "w") as io:
        io.write(nwbfile)

    with NWBHDF5IO(tmp_path / "test_result_links.nwb", "r", load_namespaces=True) as io:
        read_nwbfile = io.read()
        read_registration = read_nwbfile.lab_meta_data["atlas_registration"]
        read_localization = read_nwbfile.lab_meta_data["localization"]

        # Links resolve to the objects stored under Localization
        assert isinstance(
            read_registration.anatomical_coordinates_images["RegisteredCoords"], AnatomicalCoordinatesImage
        )
        assert isinstance(read_registration.brain_region_masks["source_region_masks"], BrainRegionMasks)
        assert (
            read_registration.anatomical_coordinates_images["RegisteredCoords"]
            is read_localization.anatomical_coordinates_images["RegisteredCoords"]
        )
        assert (
            read_registration.brain_region_masks["source_region_masks"]
            is read_localization.brain_region_masks["source_region_masks"]
        )
